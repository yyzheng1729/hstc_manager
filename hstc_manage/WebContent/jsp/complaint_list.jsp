...
<body>
	<nav class="breadcrumb">
	    ...
	    <span class="c-gray en">&gt;</span>投诉管理
	    <span class="c-gray en">&gt;</span>投诉列表
	...
	<div class="page-container">
	    <div class="mt-5">
	        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive" >
	            <thead>
		            <tr>
						<th scope="col" colspan="6">投诉列表</th>
					</tr>
	                <tr class="text-c">
	                    <th width="80">投诉编号</th>
	                    <th width="80">招聘信息标题</th>
	                    <th width="150">微信昵称（投诉用户）</th>
	...
	<script type="text/javascript">
		...
		function loading(pagenum){
			$.ajax({
				type:"POST",
				url:"/hstc_manage/display_complaint",
				dataType:"json",
				data:{
					pagenum:pagenum
				},
		...
		function displayData(data){
			...
			for( i in data ){	
				str += '<tr class="text-c">'+
				       '<td>'+data[i].complaintId+'</td>'+
				       '<td>'+data[i].tRecruitment.title+'</td>'+
				       '<td>'+data[i].tUser.nickname+'</td>'+
				       '<td class="f-14 td-manage">'+
				       '<a style="text-decoration:none" class="ml-5" onclick="del(this,'+data[i].complaintId+')" href="javascript:;" title="删除"><i style="font-size:25px" class="Hui-iconfont">&#xe6e2;</i></a>'+
				       '</td>'+
				       '</tr>';
			}
			tbody.innerHTML = str;
		...
		/*用户信息-删除*/
		function del(obj,complaintId) {
		    layer.confirm('确认要删除吗？',
		    function(index){
		        $.ajax({
		            type: 'POST',
		            url: '/hstc_manage/deleteByComplaintId',
		            dataType: 'json',
		            data:{
